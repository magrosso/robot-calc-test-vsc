...
class Application:
    ...
    @keyword
    def start_application(self, hw_tree: str):
        """Start application under test
        """
        Application.logger.info(f'Start Application with {hw_tree}')
        Application.rpa.windows_run('Calc.exe')
        Application.app_root = Application.rpa.control_window('name:Calculator')
        Application.rpa.set_global_timeout(5.5)
        Application.init_application(self)
        # Application.lasx = Application.lib.control_window('executable:LMSApplication.exe')
